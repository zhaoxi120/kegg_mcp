...
SERVER_INSTRUCTIONS = (
    ...
    "state is unknown and probe_kegg_connectivity before network-dependent work. Explicit file "
    "inputs may be any readable absolute local regular file, and explicit output directories may "
    "be any safe new or empty absolute location. Configured roots are used only to allocate "
    "default output locations. Result identifiers are scoped to "
