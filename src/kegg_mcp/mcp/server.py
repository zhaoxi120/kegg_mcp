...
SERVER_INSTRUCTIONS = (
    ...
    "If either appears unavailable, do not infer a missing installation from the current task's "
    "tool snapshot alone. First distinguish stale discovery from an incomplete deployment using "
    "the Codex plugin and MCP inventories when they are available. An enabled suite plugin with "
    "all three registered servers is not reinstalled: restart Codex once and retry in a new task. "
    "Only when the inventory is incomplete or a component reports a concrete deployment failure, "
    "report an incomplete suite deployment and request explicit permission once to install or "
    "repair the complete repository suite. If the user declines that action, remain stopped until "
    "a user-selected route supplies supported KO evidence. After the "
