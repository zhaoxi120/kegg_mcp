...
def test_module_comparison_recomputes_accepted_only_outcomes_under_one_graph() -> None:
    ...
    assert [item.evaluation_status for item in target.comparison.outcomes] == [
        ModuleEvaluationStatus.COMPLETE,
        ModuleEvaluationStatus.INCOMPLETE,
        ModuleEvaluationStatus.INCOMPLETE,
    ]
    assert target.comparison.complete_in_set_indexes == (0,)
    assert target.comparison.incomplete_in_set_indexes == (1, 2)
    assert target.comparison.outcomes_differ is True
    assert all(item.required_block_count == 2 for item in target.comparison.outcomes)
...
def test_multiple_module_targets_preserve_caller_order_and_shared_context() -> None:
    ...
    assert all(
        ...
        for outcome in result.targets[0].comparison.outcomes
    )
...
def test_not_evaluable_module_outcomes_never_claim_coverage() -> None:
    ...
    assert target.comparison.not_evaluable_in_set_indexes == (0, 1, 2)
    assert target.comparison.outcomes_differ is False
    assert all(outcome.block_coverage is None for outcome in target.comparison.outcomes)
