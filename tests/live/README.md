...
Pull-request CI makes 20 serialized requests: five each for `INFO`, `GET`, `LINK`, and `CONV`.
...
The default is five requests per operation. Set `KEGG_MCP_LIVE_REQUESTS_PER_OPERATION` to an
integer from 1 through 30 for an explicitly authorized manual run.
...
GitHub Actions runs the 20-request campaign for pull requests and manual workflow dispatches. A
