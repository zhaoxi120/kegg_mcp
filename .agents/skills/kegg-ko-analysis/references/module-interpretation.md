...
- Preserve AND, OR, optional, parenthesized, and referenced-module semantics. If unsupported,
  malformed, unresolved, cyclic, or unavailable content affects evaluation, report
  `partially_evaluable` or `not_evaluable` with the reason; never silently discard it.
